...
pub fn main() !void {
    ...
    const light_position = rt_zig.Tuple.point(5, 5, -2);
    world.add_light(rt_zig.Light.spot(
        light_position,
        rt_zig.Color.WHITE,
        rt_zig.Tuple.vector(0, 0, 0).sub(light_position),
        std.math.pi / 8.0,
        0.5,
    ));
    ...
    var camera = rt_zig.Camera.init(1200, 900, 1, std.math.pi / 3.0);
