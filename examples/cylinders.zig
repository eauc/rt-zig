...
pub fn main() !void {
    ...
    var middle = rt_zig.Object.cylinder()
        .truncate(1, 2, true)
        .with_transform(translation(0, 0, 2).mul(rotation_x(-std.math.pi / 6.0)).mul(rotation_y(std.math.pi / 6.0)));
    middle.material.color = rt_zig.Color.init(0.1, 1, 0.5);
    middle.material.diffuse = 0.7;
    middle.material.specular = 0.3;
    middle.material.reflective = 0.3;

    const right = rt_zig.Object.cylinder()
        .made_of_glass()
    ...
    var left = rt_zig.Object.cylinder()
        .truncate(0, 2, false)
        .with_transform(translation(-1.5, 0.33, -0.75)
        .mul(scaling(0.33, 0.33, 0.33)).mul(rotation_x(-std.math.pi / 6.0)).mul(rotation_z(std.math.pi / 6.0)));
